...
def index_points(device, points, idx):
    ...
    Return:
        new_points:, indexed points data, [B, S, C] (s = n*k)
    """
    B = points.shape[0]
...
def knn_l2(device, net, k, u):
    ...
    minus_distance = 2 * torch.matmul(net, net.transpose(2, 1)) - square - square.transpose(2, 1) + torch.Tensor(
        u_block(batch_size, npoint, u)).cuda()  # (bnc * bcn) -> (b,n,n)
    _, indices = torch.topk(minus_distance, k, largest=True, sorted=False)  # (b,n,k)
